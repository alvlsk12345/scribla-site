 ...
 *
 * robots.txt и sitemap.xml сюда добавлены 13 августа 2026: до этого дня
 * они жили только на сервере, вне репозитория и вне выкладки. Правка
 * такого файла проходила мимо истории, а чистая раскатка сайта унесла бы
 * их совсем.
 *
 * favicon.ico и favicon.svg — 17 августа, по той же причине. Лежат они
 * в корне не от лени: Яндекс ищет файл с именем favicon именно там,
 * и объявленной из assets/brand ему мало. Собираются они
 * скриптом tools/build-favicon.py.
 *
 * Каталога download здесь нет намеренно: в нём лежит образ маковой
 * версии, 24 МБ двоичного файла, которым в репозитории делать нечего.
 * Раз его нет в списке, выкладка его и не тронет — образ переживает
 * любую раскатку, а меняется отдельно, через ssh. */
const TAKE = ['index.html', 'privacy.html', 'support.html', 'robots.txt',
              'sitemap.xml', 'favicon.ico', 'favicon.svg',
              '.htaccess', 'assets', 'api', 'en', 'es', 'zh'];
...
/* Рабочие тексты инструкции для модели — тем же путём и по той же
 * причине, что почта и ключи.
 *
 * В репозитории лежат запасные формулировки, короткие и общие. Рабочие
 * выкуплены живыми провалами — правило про момент времени рядом с числом,
 * обрез страницы в четыре тысячи знаков, запрет выдумывать при правке
 * прошлого ответа, — и в публичном репозитории им делать нечего.
 *
 * Молчание значит «оставь как есть». `version` обязателен: он уходит
 * в диагностику приложения, и по нему видно, какая редакция дала какой
 * ответ. Ноль в той строке означает, что файла нет вовсе и режим AI идёт
 * на запасных текстах — это надо замечать, а не гадать.
 */
$prompt = $sent['prompt'] ?? null;
if (is_array($prompt)) {
    if ((int) ($prompt['version'] ?? 0) < 1) {
        say(400, ['error' => 'В текстах инструкции нет version']);
    }

    /* Берём только строки и номер редакции: сюда приходит то, что уйдёт
     * в чужую модель, и лишнее поле в этом файле — не опечатка, а кусок
     * инструкции, которого никто не писал. */
    $clean = ['version' => (int) $prompt['version']];
    foreach ($prompt as $name => $value) {
        if ($name !== 'version' && is_string($value) && $value !== '') {
    ...
    $json = json_encode($clean, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
    if ($json === false || @file_put_contents($file, $json, LOCK_EX) === false) {
        say(500, ['error' => 'Не удалось записать тексты инструкции']);
    }
    @chmod($file, 0600);
    $log[] = 'инструкция обновлена: редакция ' . $clean['version']
        . ', кусков ' . (count($clean) - 1);
}
