 ...
 *
 * Языков стало четыре, и позиционные аргументы под них не годятся:
 * четыре подряд идущие фразы в вызове читаются как загадка, а перепутать
 * в них порядок можно молча. Поэтому набор приходит массивом по ключу
 * языка, а чего в нём нет — берётся русским: он на сайте есть всегда. */
function pick(array $in, array $say): string
{
    $lang = substr((string) ($in['lang'] ?? ''), 0, 2);
    return $say[$lang] ?? $say['ru'];
}

/* Как назвать страницу в письме владельцу. Отдельно от pick(): здесь
 * язык не отвечает человеку, а помечает заявку в ящике, и написано это
 * по-русски — письма читает владелец. */
function lang_name(array $in): string
{
    $names = ['ru' => 'русская', 'en' => 'английская',
              'es' => 'испанская', 'zh' => 'китайская'];
    $lang = substr((string) ($in['lang'] ?? ''), 0, 2);
    return $names[$lang] ?? 'русская';
}
