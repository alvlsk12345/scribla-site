...
$system = $lastSystem();
/* Уточнение по существу: курса евро в прошлом ответе не было, и без
 * нового поиска модели остаётся только отказаться — ровно это человек
 * и получил 17 августа 2026: «Для ответа на этот вопрос нужен новый
 * поиск». Теперь такое уточнение ищет заново. */
$ok('уточнение по существу — состояние refine_found',
    ($json['trace']['state'] ?? '') === 'refine_found');
$ok('уточнение по существу — поиск был', $searches() !== []);
$ok('уточнение по существу — искали с предметом прошлого вопроса',
    str_contains(implode(' ', $searches()), 'курс доллара'));
$ok('уточнение — прошлый обмен приложен', str_contains($system, 'ПРЕДЫДУЩИЙ ОБМЕН'));
$ok('уточнение — про отсутствие интернета ни слова',
    !str_contains($system, 'Интернета у тебя нет'));
$ok('уточнение с поиском — сказано отвечать по свежим страницам',
    str_contains($system, 'Мы поискали заново'));
$ok('уточнение с поиском — найденное приложено',
    str_contains($system, 'НАЙДЕНО В ИНТЕРНЕТЕ'));

/* А правка формы по-прежнему обходится без сети: искать по «сделай
 * короче» нечего, и лишний поход наружу стоил бы секунды на пустом
 * месте. */
$scene('ok');
[$code, $json] = $post(array_merge($base, [
...
$system = $lastSystem();
$ok('правка формы — состояние refine', ($json['trace']['state'] ?? '') === 'refine');
$ok('правка формы — заново не ищем', $searches() === []);
$ok('правка формы — запрет выдумывать числа',
    str_contains($system, 'НЕ ВЫДУМЫВАЙ новых чисел'));
$ok('правка формы — источники прошлого ответа сохранены',
